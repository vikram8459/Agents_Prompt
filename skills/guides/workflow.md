...
2. **Define contracts first.** ABCs and Protocols in `core/interfaces/`, before implementations. See [OOP and contracts](../architecture/oop-and-contracts.md).
3. **Place code in the right layer.** See [layout](../architecture/layout.md).
4. **Decide monolith or service** if that is in question. See [service boundaries](../architecture/service-boundaries.md).
