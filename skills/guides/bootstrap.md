...
Create the layer and test directories from [layout.md](../architecture/layout.md), each with an `__init__.py`. Do not create a layer the project has no use for yet; an empty `infrastructure/messaging/` is clutter that implies a queue exists.

Create `.pre-commit-config.yaml` from the snippet in [tooling.md](tooling.md), then run `pre-commit install` once. This requires the project to be a git repository; if it is not, say so rather than running `git init` unprompted.
